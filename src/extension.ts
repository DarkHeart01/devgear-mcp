...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('devgear.webview', webviewProvider)
    );
    ...
    const commands = [
        ...
        vscode.commands.registerCommand('devgear.dockerize', async (uri?: vscode.Uri) => {
        ...
        vscode.commands.registerCommand('devgear.deployVercel', async (uri?: vscode.Uri) => {
            const targetPath = uri?.fsPath || vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            if (targetPath) {
    ...
    orchestrator.initialize().then(() => {
        vscode.window.showInformationMessage('DevGear: MCP servers initialized successfully!');
    }).catch(error => {
        vscode.window.showErrorMessage(`DevGear: Failed to initialize MCP servers: ${error.message}`);
    });
